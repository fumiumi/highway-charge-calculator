...
## 料金計算CLIの単価・式（2026-09-25確認）

- NEXCO東日本「高速自動車国道料金の計算式」
  https://www.driveplaza.com/assets/pdf/etc/dis/etc_dis_kanetsu/kanetsu_feecalculation.pdf
  普通区間24.6円/km、大都市近郊29.52円/km、車種比率0.8/1/1.2/1.65/2.75、
  固定額150円、100/200kmの逓減係数、税1.10、料金端数処理を照合。
  普通区間単価と計算の基本処理はユーザー提示コードを採用。
- NEXCO中日本「料金の計算方法を教えてください」
  https://highwaypost.c-nexco.co.jp/faq/toll/findout/23.html
  距離料金、固定額、税、普通区間と大都市近郊区間の水準の補助確認。
- NEXCO東日本「圏央道の料金について」（2016年資料）
  https://www.e-nexco.co.jp/rest/pressroom/press_release/head_office/h28/0301/pdfs/01.pdf
  路線によって長距離逓減の取扱いが異なることを確認。一律のサンプル計算式を
  すべての路線の公式料金として表示しない理由。個別特例のマスターには未採用。

CLIは `sample-distance-tariff-v1` として提示された式を区分別距離へ拡張した概算。
複数区分では区分距離を各0.1kmに丸め、合計距離の逓減係数を距離料金全体へ適用する。
これは路線ごとの正式な課金・割引の再現を保証しない。未確認・特別区間は単価を
推測せず、CLIで指定した場合のみ「利用者の仮定」として使う。元の料金区分は変更しない。
